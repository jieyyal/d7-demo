...
_sdi.notify = function(message){
  try {
    ...
    var notify = window._satellite ? window._satellite.notify : window._slimer.notify;
    notify(message, 1);
  }
}
...
_sdi.yt = {
  ...
  checkIframes: function(){
    ...
    jQuery('iframe[src*="youtube"][data-sdiyt!="true"]').each(function(idx, ifr) {
      var vid = ifr.src.match(/h?t?t?p?s?\:?\/\/www\.youtube(-nocookie)?\.com\/embed\/([\w-]{11})(?:\?.*)?/), id;
      if(vid){
...
if(typeof _satellite != 'undefined'){
  _satellite.addEventHandler(window, "load", function(){
    _satellite.notify('YT - Satellite DOM Ready',1);
    _sdi.yt.init();
    _sdi.yt.ready('dom');
  });
}
else if(typeof _slimer != 'undefined'){
  _slimer.addEventHandler(window, "load", function(){
    _slimer.notify('YT - Slimer DOM Ready',1);
    _sdi.yt.init();
    _sdi.yt.ready('dom');
  });
}
